...
#pragma omp parallel
  {
    ...
    // This potentially throws away the last points
#pragma omp for simd safelen(2) reduction(+ : real, imag, real1, imag1)
    for (uint64_t i = 0; i < chunks; i += 2) {
      real += (double)signal[i] * cos[i % Q];
      imag += (double)signal[i] * sin[i % Q];
      real1 += (double)signal[i + 1] * cos[(i + 1) % Q];
      imag1 += (double)signal[i + 1] * sin[(i + 1) % Q];
    }
  }
